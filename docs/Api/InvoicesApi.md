...
| [**getInvoiceDocuments()**](InvoicesApi.md#getInvoiceDocuments) | **GET** /v3/client/invoices/{id}/documents | Get documents for an Invoice |
...
```php
getInvoiceDocuments($id): \SynergiTech\Cinolla\Model\Document[]
```
...
try {
    $result = $apiInstance->getInvoiceDocuments($id);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling InvoicesApi->getInvoiceDocuments: ', $e->getMessage(), PHP_EOL;
}
```
...
$bookingId = 'bookingId_example'; // string | Filter invoices based on a specific Booking ID (or hash).
...
| **bookingId** | **string**| Filter invoices based on a specific Booking ID (or hash). | [optional] |
